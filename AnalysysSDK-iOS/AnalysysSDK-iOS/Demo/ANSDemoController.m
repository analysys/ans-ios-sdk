...
@interface ANSDemoController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
//                NSString *serverUrl = @"https://serverDomain";
                NSString *serverUrl = @"https://sdk.analysys.cn:4089";
        ...
        //  为了方便查看现使用随机事件做Demo
        NSString *event = [NSString stringWithFormat:@"event_%d",arc4random()%100];
        switch (indexPath.row) {
            case 0: {
        ...
        switch (indexPath.row) {
            case 0: {
                //  为了方便查看现使用随机id做Demo
                NSString *distinct_id = [NSString stringWithFormat:@"WeChatID_%d",arc4random()%1000];
